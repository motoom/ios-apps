...
class GameController: UIViewController {
    ...
    var pourTimer: Timer? = nil
    var initiateDrainTimer: Timer? = nil
    var drainTimer: Timer? = nil
    var initiateNewGameTimer: Timer? = nil

    // MARK: Vessel initialisation.

    override func viewDidLoad() {
        super.viewDidLoad()
        vesselViews[3].isHidden = true
        vesselViews[4].isHidden = true
        activeVessels = 3
        initiateNewGameTimer = Timer.scheduledTimer(timeInterval: 0.2, target: self, selector: #selector(GameController.initiateNewGame), userInfo: nil, repeats: false)
        }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let settings = segue.destination as? SettingsController {
            settings.difficulty = difficulty
            settings.vesselCount = activeVessels
            }
        }

    @IBAction func backFromSettings(_ segue: UIStoryboardSegue) {
        if let settings = segue.source as? SettingsController {
            difficulty = settings.difficulty
            activeVessels = settings.vesselCount
            switch activeVessels {
                case 4:
                        vesselViews[3].isHidden = false
                        vesselViews[4].isHidden = true
                case 5:
                        vesselViews[3].isHidden = false
                        vesselViews[4].isHidden = false
                default:
                        vesselViews[3].isHidden = true
                        vesselViews[4].isHidden = true
                }
    ...
    @IBAction func three(_ sender: UIBarButtonItem) {
        vesselViews[3].isHidden = true
        vesselViews[4].isHidden = true
    ...
    @IBAction func four(_ sender: UIBarButtonItem) {
        vesselViews[3].isHidden = false
        vesselViews[4].isHidden = true
    ...
    @IBAction func five(_ sender: UIBarButtonItem) {
        vesselViews[3].isHidden = false
        vesselViews[4].isHidden = false
    ...
    @IBAction func pan(_ sender: UIPanGestureRecognizer) {
        ...
        if sender.state == .began {
            let point = sender.location(ofTouch: 0, in: nil)
            ...
            if let src = sourceSelected, let dst = destinationSelected {
    ...
    func pourValid(_ src: Int, _ dst: Int) -> Bool {
        if src == dst {
    ...
    func initiatePouring(_ src: Int, _ dst: Int) {
        ...
        // Start pouring timer.
        if pourTimer == nil {
            pourTimer = Timer.scheduledTimer(timeInterval: 0.05, target: self, selector: #selector(GameController.pourTick), userInfo: nil, repeats: true)
            }
        }
        ...
        if vessels[source].contents <= CGFloat(sourceFinalContents) ||
            vessels[destination].contents >= CGFloat(destinationFinalContents) {
                ...
                if let win = winningVessel {
                    ...
                    initiateDrainTimer = Timer.scheduledTimer(timeInterval: 1.5, target: self, selector: #selector(GameController.initiateDrain), userInfo: nil, repeats: false)
                    }
                }
        }
    ...
    func initiateDrain() {
        ...
        drainTimer = Timer.scheduledTimer(timeInterval: 0.05, target: self, selector: #selector(GameController.drainTick), userInfo: nil, repeats: true)
        }
    ...
    func drainTick() {
        ...
        if totalContents <= 0 {
            stopDraining()
            // Schedule a new game.
            initiateNewGameTimer = Timer.scheduledTimer(timeInterval: 3, target: self, selector: #selector(GameController.initiateNewGame), userInfo: nil, repeats: false)
            }
        }
    ...
    @IBAction func backFromPreview(_ segue: UIStoryboardSegue) {
        }
